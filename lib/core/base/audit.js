...
/**
 * Build and set the previous locale.
 */
...
Audit.prototype._resetLocale = function() {
	// If no previous locale exists, then we haven't applied one and the default is still in-tact.
...
/**
 * Merge the two check locales, favoring `b`.
 *
...
const mergeCheckLocale = (a, b) => {
	let { pass, fail } = b;
	// If the message(s) are Strings, they have not yet been run thru doT (which will
	// return a Function).
		...
		messages: {
			...
			incomplete:
				typeof a.messages.incomplete === 'object'
					? // TODO: for compleness-sake, we should be running incomplete messages thru
					  // doT as well. This was out-of-scope for runtime localization, but should
